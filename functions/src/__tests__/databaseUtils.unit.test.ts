...
describe('Database Utils (Unit Tests)', () => {
  ...
  describe('isDevUtilitiesAvailable', () => {
    ...
    it('should return false when neither condition is met', () => {
      process.env.NODE_ENV = 'production';
      process.env.ENABLE_DEV_UTILITIES = 'false';

      expect(isDevUtilitiesAvailable()).toBe(false);
    });
  });
  ...
  describe('wipeDatabase', () => {
    ...
    it('should handle empty collections gracefully', async () => {
      // Database should already be empty from beforeEach
      const db = admin.firestore();
      const usersSnapshot = await db.collection('users').get();
      const groupsSnapshot = await db.collection('groups').get();
  ...
  describe('seedDatabase', () => {
    ...
    it('should seed database with custom options', async () => {
      ...
      expect(result.userGroups.length).toBeLessThanOrEqual(6); // 3 users * 2 groups max
    });
    ...
    it('should create user-group relationships', async () => {
      ...
      expect(result.userGroups.length).toBeGreaterThan(0);
      ...
      for (const userGroup of result.userGroups) {
        ...
        expect(userGroup.balance).toBe(0);
  ...
  describe('resetDatabase', () => {
    ...
    it('should reset database with default options', async () => {
      ...
      expect(result).toBeDefined();
      expect(result.wipeResult).toBeDefined();
    ...
    it('should reset database with custom options', async () => {
      ...
      expect(result.wipeResult.totalDeleted).toBeGreaterThan(0);
      expect(result.seedResult.users.length).toBe(1);
      expect(result.seedResult.groups.length).toBe(1);
    });
    ...
    it('should handle reset with no seeding', async () => {
      ...
      expect(result.wipeResult.totalDeleted).toBeGreaterThan(0);
      expect(result.seedResult.users.length).toBe(0);
      expect(result.seedResult.groups.length).toBe(0);
    });
  });
  ...
  describe('getDatabaseStats', () => {
    it('should return stats for empty database', async () => {
      ...
      expect(stats.userGroups).toBe(0);
      expect(stats.buckets).toBe(0);
      expect(stats.transactions).toBe(0);
      expect(stats.consumption).toBe(0);
      expect(stats.joinRequests).toBe(0);
    });
    ...
    it('should return stats for populated database', async () => {
      ...
      expect(stats.userGroups).toBeGreaterThan(0);
      expect(stats.buckets).toBe(0); // No buckets by default
      expect(stats.transactions).toBe(0); // No transactions by default
      expect(stats.consumption).toBe(0); // No consumption by default
      expect(stats.joinRequests).toBe(0); // No join requests by default
    });
